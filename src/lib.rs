...
//!
//! Currently, **every message that has at least one field must include an `unknown_fields` field**.
//! It backs the catch-all arm of the generated decoder that skips over unrecognized fields, so
//! without it the derived `decode` implementation fails to compile with a non-exhaustive `match`
//! error.  (A message with no fields at all does not need one.)  Code generated by `femtopb-build`
//! always adds this field for you.  For example, omitting it does not compile:
//!
//! ```compile_fail
//! #[derive(Clone, femtopb::Message)]
//! pub struct Person<'a> {
...
//!     #[femtopb(string, tag = 2)]
//!     pub name: &'a str,
//!     // note: no `#[femtopb(unknown_fields)]` field
//! }
//! ```
//!
